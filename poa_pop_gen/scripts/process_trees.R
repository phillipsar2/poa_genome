...
# return vector of features/attributes
...
# ITS tree only -- Reroot tree to Poa_wolfii_G1065_S5800 (node 160)
#final_tree <- phytools::reroot(mytree, 160)

# with treeio
ttest <- root(mytree, outgroup = "Poa_wolfii_G1065_S5800", edgelabel = TRUE)

# with phytools
testree <- as.phylo(x)
ftest <- phytools::reroot(testree, 160)

# convert back to tree data
fftest <- as.treedata(ftest)

branchsupport <- as.matrix(matrix(nrow = length(x$label), ncol = 2))
tip_metadata[,1] <- x$label
...
t <- ggtree(ttest) +
  ...
  ## ITS: x=, y=0
  ## TLF: x=0.04, y=0
  geom_treescale(x=0.09, y=0, fontsize = 2) + # scale branch lengths
  geom_tiplab(size = 2) +
  ## ITS: 0, 0.1
  ## ETS: 0.2
  ## TLF: 0.5
  ggplot2::xlim(0, 0.1) +
  geom_nodelab(aes(x=branch, label=round(as.numeric(prob),2)), # add branch support values
              size = 1.5, vjust = -.5)
...
t %<+% tip_metadata + 
  ...
  # Highlight clades
  ## ITS: 130 & 142, extend = 0.015, extend = 0.015
  ## ETS: 139 & 176, extend = 0.08, extend = 0.09
  ## TLF: 152 & 166, extend = 0.012, extend = 0.01
  geom_hilight(node = 130, fill = "darkgray", alpha = 0.3, extend = 0.015) +
  geom_hilight(node = 142, fill = "darkgray", alpha = 0.3, extend = 0.015)


ggsave("~/poa/Trees/ITS.tree.03022022.jpeg", width = 8.5, height = 11, dpi = 300, units = "in" )
